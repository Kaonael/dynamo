...
fn detect_bpe_pattern(directory: &Path) -> Result<&'static str> {
    let model_type: String =
        crate::file_json_field(&directory.join("config.json"), "model_type")
            .map_err(|err| Error::msg(format!("Failed to read model_type from config.json: {err}")))?;
...
fn load_special_tokens(directory: &Path, num_base_tokens: usize) -> Result<FxHashMap<String, u32>> {
    ...
    let contents = std::fs::read_to_string(&config_path).map_err(|err| {
        Error::msg(format!(
            "Failed to read tokenizer_config.json: {err}"
        ))
    })?;

    let config: serde_json::Value = serde_json::from_str(&contents).map_err(|err| {
        Error::msg(format!(
            "Failed to parse tokenizer_config.json: {err}"
        ))
    })?;
