...
//! Keep the relay and global-router on one generated transport contract so
...
/// Share the cuckoo and CKF1 implementation across relay and router so both
/// sides agree on membership, recovery, and wire compatibility — and so neither
/// side has to modify `dynamo-kv-router` to get it.
pub mod cuckoo;
