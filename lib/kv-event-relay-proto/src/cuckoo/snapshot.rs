...
fn parse_header(bytes: &[u8]) -> Result<ParsedHeader, SnapshotError> {
    ...
    if bytes[8] != 16 || bytes[9] != SLOTS as u8 || bytes[10] != 0 {
    ...
    B: AsRef<[u8]>,
{
    ...
    for chunk in chunks {
        if let Some(done) = assembler.push(chunk.as_ref())? {
            return Ok(done);
        }
    }
...
pub fn apply_delta(
    ...
    for e in entries.chunks_exact(entry_len) {
        ...
        let old_slots = filter.bucket_slots(b);
        ...
        for (s, slot) in new_slots.iter_mut().enumerate() {
            ...
            new_count += usize::from(fp != 0);
            *slot = fp;
        }
        filter.set_bucket_slots(b, &new_slots);
