...
pub struct DcModelMetricsSnapshot {
    /// Keep the DC identity with the snapshot so the router can score it
    /// without consulting external state.
    pub dc_id: String,
    /// Keep the model identity with the snapshot so the score is attributable.
    pub model_id: String,
    ...
    /// Queue depth is the simplest congestion signal and stays cheap to export.
    pub queue_depth: u32,
    ...
    /// GPU utilization stays in the snapshot because the router needs a live
    /// capacity signal.
    pub gpu_util_pct: Option<f32>,
    /// KV utilization is included when capacity is known so routing can avoid
    /// hot models before they saturate.
...
impl DcModelMetricsSnapshot {
    /// Provide a neutral snapshot so the router can learn the (DC, model)
    /// tuple without biasing decisions before signals arrive.
