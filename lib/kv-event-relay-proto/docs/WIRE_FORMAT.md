...
Exact byte layouts for the relay → global-gateway transport. All multi-byte
...
- `seq: uint64` — monotonic publication counter used for stream diagnostics;
  CBI1 epoch continuity independently protects filter application.
...
| 10 | `WORKER_COUNT` | healthy model worker count |
| 11 | `FRONTEND_HEALTHY` | frontend readiness encoded as 0 or 1 |
...
## Filter payload (`wire/images.rs`)

`FilterUpdate.payload` carries a CBI1 snapshot chunk or delta. Both frame kinds
use a fixed 48-byte header containing the format identity, model-lane epoch,
and body checksum. Snapshot bodies contain bounded ranges of dense `u64`
bucket words; delta bodies contain absolute `(bucket: u32, value: u64)` images
and the epoch they extend.

The complete byte layout and consumer invariants are specified in
[CUCKOO.md](CUCKOO.md#cbi1-header). Golden-byte and malformed-frame tests live
in `src/wire/images.rs`.
